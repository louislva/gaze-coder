const input2 = `sfDRhjhHsHhgWPJvPmmQnmPqnW
...
for (let i = 0; i < lines.length; i += 3) {
  ...
  const alphabet = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ";
  const AlphabetCounts = [...alphabet].map((letter) => [
    letter,
    group.filter((line) => line.includes(letter)).length,
  ]);
  const firstValueWithCountThree = AlphabetCounts.find(
    ([letter, count]) => count === 3
  );
